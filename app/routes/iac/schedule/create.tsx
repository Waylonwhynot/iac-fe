...
import { Mission, PeriodicMission, PeriodicMissionCreation, Template, TemplateCreation } from '~/generated';
...
export const loader: LoaderFunction = async ({ request }) => {
    ...
    if (query.has('from')) {
        ...
        if (query.get('type') === 'schedule') {
            const from = await scheduleApi.withMiddleware(server(request)).getPeriodicMission({ id: fromId });
            return json(from);
        }
...
const getScheduleType = (value: Mission | Template | PeriodicMission | undefined) => {
...
export default function() {
    ...
    const from = useLoaderData<Mission | Template | PeriodicMission | undefined>();
    const createPeriodicMission = useAsync(scheduleApi.createPeriodicMission.bind(scheduleApi));
    ...
    useEffect(() => {
        if (createPeriodicMission.state === 'COMPLETED') {
            navigate(`/iac/schedule/${createPeriodicMission.data?.id}`);
        }
    }, [createPeriodicMission.state]);
    ...
    const handleSubmit = (values: PeriodicMissionCreation) => {
        // console.log(values);
        if (values.crontab && !values.crontab.timezone) {
            values.crontab.timezone = 'UTC';
        }
        createPeriodicMission.run({ periodicMissionCreation: values });
    };
